...
const STD_CFG: &str = r#"# Config File, generated at build time.
...
const ROCKET_CFG: &str = r#"# Config File, generated at build time.
[development]
address = "localhost"
port = 8000
workers = 12
...
fn main() {
    ...
    // Export ZK.toml
    let mut destfile =
        File::create(Path::new(&output_path).join("ZK.toml")).expect("Error writing config file!");
    let mut devfile =
        File::create(Path::new(&env::current_dir().unwrap()).join("ZK.toml")).unwrap();
    ...
    // Export Rocket.toml
    let mut destfile = File::create(Path::new(&output_path).join("Rocket.toml"))
        .expect("Error writing config file!");
    let mut devfile =
        File::create(Path::new(&env::current_dir().unwrap()).join("Rocket.toml")).unwrap();
